...
public class TestTwitterMetadata
{
 /*   private static final TwitterTableHandle NUMBERS_TABLE_HANDLE = new TwitterTableHandle("example", "numbers");
    private TwitterMetadata metadata;

    @BeforeMethod
    public void setUp()
            throws Exception
    {
        URL metadataUrl = Resources.getResource(TestTwitterClient.class, "/example-data/example-metadata.json");
        assertNotNull(metadataUrl, "metadataUrl is null");
        TwitterClient client = new TwitterClient(new TwitterConfig().setMetadata(metadataUrl.toURI()), CATALOG_CODEC);
        metadata = new TwitterMetadata(client);
    }
    ...
    public void testGetColumnHandles()
    {
        ...
        assertEquals(metadata.getColumnHandles(SESSION, NUMBERS_TABLE_HANDLE), ImmutableMap.of(
                "text", new TwitterColumnHandle("text", createUnboundedVarcharType(), 0),
                "value", new TwitterColumnHandle("value", BIGINT, 1)));

        // unknown table
        try {
            metadata.getColumnHandles(SESSION, new TwitterTableHandle("unknown", "unknown"));
            fail("Expected getColumnHandle of unknown table to throw a TableNotFoundException");
        }
        catch (TableNotFoundException expected) {
        }
        try {
            metadata.getColumnHandles(SESSION, new TwitterTableHandle("example", "unknown"));
            fail("Expected getColumnHandle of unknown table to throw a TableNotFoundException");
        }
        catch (TableNotFoundException expected) {
        }
    ...
    public void getTableMetadata()
    {
        ...
        assertNull(metadata.getTableMetadata(SESSION, new TwitterTableHandle("unknown", "unknown")));
        assertNull(metadata.getTableMetadata(SESSION, new TwitterTableHandle("example", "unknown")));
        assertNull(metadata.getTableMetadata(SESSION, new TwitterTableHandle("unknown", "numbers")));
    }
    ...
    {
        assertEquals(metadata.getColumnMetadata(SESSION, NUMBERS_TABLE_HANDLE, new TwitterColumnHandle("text", createUnboundedVarcharType(), 0)),
                new ColumnMetadata("text", createUnboundedVarcharType()));
